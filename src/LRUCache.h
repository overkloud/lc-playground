...
public:
    struct node
    {
        ...
        string toString()
        {
            //string s = "key: " + key + " val: " + val + " time: " + time;
            stringstream ss;
            ss << "keys: " << key << " val: " << val << " time: " << time;
            return ss.str();
        }
    };
    ...
    void heapify(ll time)
    {
        ...
        while (i < n)
        {
            ...
            {
                int min = 0;
                if (q[right]->time < q[left]->time)
                    min = right;
                else min = left;
                if (q[min]->time < q[i]->time)
                    swap = min;
            }
            else if (left < n)
            {
                if (q[left]->time < q[i]->time)
                    swap = left;
            }
            if (swap != i)
            {
...
class LRUCache_fast : public LRUCache {
public:
    struct CacheEntry
    {
    public:
    ...
    LRUCache_fast(int capacity) {
        m_capacity = capacity;
    }

    LRUCache_fast() : LRUCache_fast(0)
    {
    }
    ...
    {
        testUtil::timer t;
        LRUCache_fast cache(2048);
        t.start();
        cache.perfTest2();
        cout << "fast version spent " << t.stop() << endl;
    }
private:
